...
void move_robot(const std::shared_ptr<rclcpp::Node> node)
{
    ...
    if(!gripper_within_bounds | !arm_within_bounds){
    ...
    if(arm_plan_success && gripper_plan_success){
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "One o more planner failed");
        return;
    }
...
std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("simple_moveir_interface");
move_robot(node);
rclcpp::spin(node);
rclcpp::shutdown();
return 0;
