...
class SimpleParameter : public rclcpp::Node
{
    ...
    SimpleParameter() : Node("simple_parameter"){
        ...
        declare_parameter<std::string>("simple_string_param", "");
	...
	rcl_interfaces::msg::SetParametersResult	paramChangeCallback(const std::vector<rclcpp::Parameter>& params){
			...
			for (const auto& param: params){
				if (param.get_name() == "simple_int_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_INTEGER){
					RCLCPP_INFO_STREAM(this->get_logger(), "Param simple_int_param changed! New value is: " << param.as_int());
					result.successful = true;
				}
				if(param.get_name() == "simple_str_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_STRING){
					RCLCPP_INFO_STREAM(this->get_logger(), "Param simple_str_param changed! New value is: " << param.as_string());
					result.successful = true;
				}
				}
			return result;
